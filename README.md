...
> Add screenshots to `docs/` and link them here. Suggested set:
> - `docs/report-summary.png` — a rendered report header with overall confidence and data quality
> - `docs/provenance.png` — a claim showing its source tier and `claim_id` lineage
> - `docs/run-metadata.png` — the cost, token, and tier-coverage summary for a run

A trimmed example of what a run produces:

```text
Apple Inc. — due-diligence report
overall confidence: 81 / 100        data quality: medium
financials (EDGAR, primary tier):
  revenue        $416.16B (FY2025)   confidence: high
  net income     $112.01B (FY2025)   confidence: high
sources: primary 27% · reputable 20% · community 13% · unknown 38%
provenance: 72/72 synthesis references resolve · 0 dangling
gaps: revenue_growth
```
